...
const fetchWalletTokenDetails = async (walletStore: ReturnType<typeof useWalletStore>) => {
  if (!walletStore.isWalletDataLoaded || walletStore.getUTxOsAtWallet().length === 0) return undefined;
  const assets = getAssetOfUTxOs(walletStore.getUTxOsAtWallet());
  const details = await TokenMetadataFrontEndApiCalls.getAssetsWithDetailsApi(assets);
...
const fetchOtcTokensWithMetadata = async (): Promise<OTCEntityWithMetadata[]> => {
  ...
  return entities.reduce<OTCEntityWithMetadata[]>((acc, entity) => {
    const metadata = metadataList.find(m =>
      m.CS === entity.od_token_policy_id && m.TN_Hex === entity.od_token_tn
    );
    if (metadata) {
...
export const useHome = () => {
  ...
  const handleBtnSync = async () => {
    ...
    try {
      await OTCApi.syncWithAddressApi(
        OTCEntity,
        appState.protocol.getOTC_Net_Address(),
        appState.protocol.getOTC_NET_id_CS(),
        true
      );
      refreshList();
